...
import thePaths from '../config/paths.mjs';

const moduleAliases = thePaths;
...
const [stat, lstat, writeFile, unlink, readdir, symlink, mkdir, rmdir] = fsMethods.map((m) =>
  promisify(fs[m]),
);
...
async function linkModule(moduleName) {
  ...
  const linkExists =
    moduleExists && (await exists(path.join('node_modules', getModuleAlias(moduleName))));
  ...
  if (moduleName.match(/^@/) && !packageJson._moduleAliasIgnoreWarning) {
    console.warn(
      chalk.yellow(
        `WARNING! Using @ in front of your module name ${moduleName} may cause data loss. Please read this issue thread https://github.com/Rush/link-module-alias/issues/3 and make a backup before executing any npm/yarn commands. The issue ultimately can be blamed on npm/yarn. You've been warned.`,
      ) +
  ...
  if (moduleExists && !linkExists) {
    console.error(
      chalk.red(`Module ${moduleName} already exists and wasn't created by us, skipping`),
    );
    type = 'none';
...
async function unlinkModules() {
  ...
  const unlinkedModules = await Promise.all(
    modules.map((mod) => {
